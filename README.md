# Python Course

---

This repository is a comprehensive python course from basic to advance level. This course contains several code examples and explanation on various topics of python programming language.

## Course Curriculam

**Python_Roadmap.pdf** contains the python language topics and learning roadmap with sample project description. This is prepared by Generative AI.

## Course Description

- **01_Functions**: This contains all about functions.

- **02_Error_Exception**: This contains all about exceptions and error in Python.

- **03_Modules_Packages_Library**: This contains concepts of modules, packages and library in Python.

- **04_Strings**: This contains all about Python Strings.

- **05_File_Handling**: This contains python file handlings.

- **06_OOP_Fundamentals**: This contains python object oriented programming fundamentals.

- **07_OOP_Advance**: This contains python OOP advance concepts.

- **08_Context_Manager**: This contains python context manager concept.

- **09_Iterators_Generators_Coroutine**: This contains all about iterators, generators and coroutines in python.

- **10_Unit_Testing_Python**: This contains python unit testing modules: _doctest, unittest, pytest and nosetest_.

- **QuizProg**: This contains code snippets in different python quizes.

## Tutorials

for more comprehensive tutorial, please visit the website - https://fizzbuzzblog.vercel.app/
